...
# ======================
def tela_usuarios():
    """Mostra uma lista com todos os usuários cadastrados."""
    janela_lista = tk.Toplevel()
    janela_lista.title("Usuários Cadastrados")
    janela_lista.geometry("300x300")

    lista = tk.Text(janela_lista, width=30, height=15)
    lista.pack(pady=10)

    usuarios = obter_usuarios()
    lista.insert(tk.END, "Usuários cadastrados:\n\n")
    for u in usuarios:
        lista.insert(tk.END, f"• {u[0]}\n")

    lista.config(state="disabled")  # Apenas leitura
...
def tela_login():
    ...
    tk.Button(janela_login, text="Ver Usuários", command=tela_usuarios).pack(pady=5)
